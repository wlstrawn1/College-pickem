...
async function loadTracking({skipScoreRefresh=false}={}){
  ...
  const rows=ranked.map(e=>{
    ...
    const tbCell=`<td class="tracking-tiebreak">${showTb?`${e._tb.error} margin err<small>${htmlEscape(String(e._tb.predicted.teamA))}-${htmlEscape(String(e._tb.predicted.teamB))}</small>`:"—"}</td>`;
