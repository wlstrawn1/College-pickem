...
function updateTiebreakUI(){
  ...
  if($("clemsonScore")) $("clemsonScore").placeholder=shortTeam(g.dog);
  if($("lsuScore")) $("lsuScore").placeholder=shortTeam(g.fav);
}
...
function renderMy(){
    ...
    return `<article class="scorecard-row ${statusClass}">
      <div class="scorecard-row-main">
        ...
        <div class="scorecard-row-sub">
          ...
          <span>${htmlEscape(r.g.fav)} ${r.g.spread}</span>
          <span>${r.pts} pt${r.pts>1?"s":""}</span>
          ${scoreLine?`<span>${htmlEscape(scoreLine)}</span>`:""}
        </div>
      </div>
      ${rightSide}
    </article>`;
  ...
  host.innerHTML=`
    <div class="scorecard-stats">
      <div class="scorecard-stat"><span>Points</span><strong>${earnedPts}<small> of ${maxFinish} possible</small></strong></div>
      ...
      <div class="scorecard-stat"><span>On The Board</span><strong>${pendingPts}<small>${onBoard.length} game${onBoard.length===1?"":"s"}</small></strong></div>
    </div>
    <div class="scorecard-bar-wrap">
      ...
      <div class="scorecard-legend">
        <span><b class="tracking-dot correct"></b>${earnedPts} won</span>
        <span><b class="tracking-dot wrong"></b>${lostPts} missed</span>
        <span><b class="tracking-dot pending"></b>${pendingPts} on the board</span>
      </div>
    </div>
...
async function loadCurrentWeek(){
  ...
  if(s.exists()) weekData={id:s.id,...s.data()};
...
document.querySelectorAll("nav button[data-tab]").forEach(b=>b.onclick=async()=>{setTab(b.dataset.tab);if(b.dataset.tab==="tracking")await loadTracking();if(b.dataset.tab==="results")await refreshAutomaticScores();if(b.dataset.tab==="leaderboard")await renderSeasonLeaderboard();if(b.dataset.tab==="admin")await loadCommissionerDashboard();if(b.dataset.tab==="mypicks"){await refreshAutomaticScores();renderMy();}});
