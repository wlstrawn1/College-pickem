...
async function openEditEntryPicks(entryId){
  ...
  const gameRows=gs.map(g=>{
    ...
    return `<div class="edit-pick-row">
      <div class="edit-pick-matchup"><strong>${htmlEscape(g.dog)}</strong> vs <strong>${htmlEscape(g.fav)}</strong><span>${htmlEscape(g.fav)} ${g.spread} · ${g.points||1} pt${Number(g.points||1)!==1?"s":""}</span></div>
