...
async function updateWeeklyPersonalSummary(){
  ...
  try{
    ...
    rankEl.textContent=idx>=0?`#${idx+1} of ${ranked.length}`:(submittedAt?"—":"No entry");
  }catch(e){
    rankEl.textContent="—";
  }
}
...
function standingRowsFromTotals(totals){
  ...
  // individual week's tiebreak and champion, not the season-long ranking.
  ...
  rows.forEach((r,i)=>{
    const key=`${r.wins}|${r.losses}|${r.pushes}|${r.points}`;
