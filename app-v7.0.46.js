...
async function loadTracking({skipScoreRefresh=false}={}){
  ...
  // Group entries by raw score; if a group has more than one player AND at
  // least one of them ended up ranked top-3, tiebreak was the deciding factor
  // for that whole group — show it for everyone in that group (including
  // members it pushed just outside the top 3, so it's clear why). Anyone not
  // in a top-3-deciding tie shows a plain dash instead of noise.
  const scoreGroups=new Map();
  ranked.forEach(e=>{
    const key=e._grade.score;
    if(!scoreGroups.has(key)) scoreGroups.set(key,[]);
    scoreGroups.get(key).push(e);
  });
  const tiebreakRelevantUids=new Set();
  if(final){
    for(const group of scoreGroups.values()){
      if(group.length>1 && group.some(e=>e._rank<=3)){
        group.forEach(e=>tiebreakRelevantUids.add(e._canonicalUid||e.uid||e.id));
      }
    }
  }
  ...
  const rows=ranked.map(e=>{
    ...
    const tbNote=final&&e._tb.available?` title="Tiebreak error: ${e._tb.error}"`:"";
    const uidKey=e._canonicalUid||e.uid||e.id;
    const showTb=final&&e._tb.available&&tiebreakRelevantUids.has(uidKey);
    const tbCell=`<td class="tracking-tiebreak">${showTb?`${e._tb.error} error<small>${htmlEscape(String(e._tb.predicted.teamA))}-${htmlEscape(String(e._tb.predicted.teamB))}</small>`:"—"}</td>`;
