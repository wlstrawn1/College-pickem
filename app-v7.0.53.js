...
function sortGamesByKickoff(games){
  // "Order they will be played" = actual kickoff time, ascending. Games
  // missing a stored date (shouldn't normally happen for an ESPN-imported
  // week, but just in case) sort to the end rather than breaking the sort,
  // keeping their relative order among themselves stable.
...
function matchupDisplayNames(g){
  // Home team listed second in "X vs Y" — away @ home, matching how these
  // games are actually played. Falls back to the dog/favorite order when
  // home/away isn't known (Week 1's hand-built slate, or a game the live
  // score feed hasn't matched yet), which is harmless either way since it's
  // display-only — grading always uses g.fav/g.dog regardless of this order.
  if(g?.home && g?.away) return {first:g.away, second:g.home};
  return {first:g?.dog, second:g?.fav};
}
...
function gamesForWeek(){
  ...
  if(currentWeekId==="week-1") return orderWeekOneGames(games.length?games:baseGames);
  return sortGamesByKickoff(games);
}
    ...
    return `<article class="game ${g.points>=3?"featured-game":""}">
      ...
      <div class="game-head">
        <div class="game-meta">
          ...
          <b>${matchupDisplayNames(g).first} <span class="vs-word">vs</span> ${matchupDisplayNames(g).second}</b>
  ...
  const rowHtml=(r,graded)=>{
    ...
    return `<article class="scorecard-row ${statusClass}">
      <div class="scorecard-row-main">
        <div class="scorecard-row-matchup">${htmlEscape(matchupDisplayNames(r.g).first)} <span class="vs-word">vs</span> ${htmlEscape(matchupDisplayNames(r.g).second)}</div>
...
async function saveImportedSlateDraft(){
  ...
  for(const c of selected){
    ...
    const game={id:c.id,feedEventId:c.feedEventId,fav,dog,spread:-Math.abs(rawSpread),points,date:c.date||null,network:c.network||"",home:c.home||"",away:c.away||"",lineSource:c.hasLine?"ESPN":"Commissioner",lineConfirmed:true};
    games.push(game);
...
function mergeScoreFeedIntoGames(games,events){
  return games.map(game=>{
    ...
    if(!next.home && !next.away){
      // Backfill home/away the same way we backfill kickoff time — this is
      // what lets already-imported weeks (built before home/away was tracked)
      // pick up the correct "away vs home" display order automatically once
      // the live score feed matches them, with no manual re-import needed.
      if(fav?.homeAway==="home"){ next.home=game.fav; next.away=game.dog; }
      else if(dog?.homeAway==="home"){ next.home=game.dog; next.away=game.fav; }
    }
...
async function loadTracking({skipScoreRefresh=false}={}){
  ...
  const heads=gs.map(g=>{
    ...
    return `<th class="tracking-matchup-head" title="${g.dog} vs ${g.fav} — ${g.fav} ${g.spread}"><span class="tracking-matchup-name">${matchupDisplayNames(g).first}<span class="tracking-vs">vs</span>${matchupDisplayNames(g).second}</span><span class="tracking-matchup-meta">${g.fav} ${g.spread} · ${g.points||1} pt${Number(g.points||1)!==1?"s":""}</span><span class="tracking-game-status ${feedState}">${status}</span>${score?`<span class="tracking-live-score">${score}</span>`:""}<span class="tracking-popularity"><b>${shortTeam(g.fav)}</b> ${pop.favPct}% · <b>${shortTeam(g.dog)}</b> ${pop.dogPct}%</span></th>`;
  }).join("");
...
async function openEditEntryPicks(entryId){
  if(profile?.role!=="admin"||!entryId) return;
  const panel=$("editEntryPicksPanel");
  if(!panel) return;
  panel.hidden=false;
  panel.innerHTML='<p class="helper">Loading this player\'s picks…</p>';
  panel.scrollIntoView({behavior:"smooth",block:"nearest"});
  const ref=doc(db,"weeks",currentWeekId,"entries",entryId);
  const snap=await getDoc(ref);
  if(!snap.exists()){panel.innerHTML='<p class="helper">That entry no longer exists.</p>';return;}
  const entry={id:snap.id,...snap.data()};
  const directory=playerDirectoryCache||new Map();
  const identity=resolveEntryIdentity(entry,directory);
  const name=identity.name||entry.name||"Unnamed Player";
  const gs=gamesForWeek();
  const tb=tiebreakerGame();
  const gameRows=gs.map(g=>{
    const current=entry.picks?.[g.id]||"";
    return `<div class="edit-pick-row">
      <div class="edit-pick-matchup"><strong>${htmlEscape(matchupDisplayNames(g).first)}</strong> vs <strong>${htmlEscape(matchupDisplayNames(g).second)}</strong><span>${htmlEscape(g.fav)} ${g.spread} · ${g.points||1} pt${Number(g.points||1)!==1?"s":""}</span></div>
      <select data-pick-game="${htmlEscape(g.id)}">
        <option value="" ${!current?"selected":""}>No pick</option>
        <option value="${htmlEscape(g.dog)}" ${current===g.dog?"selected":""}>${htmlEscape(g.dog)}</option>
        <option value="${htmlEscape(g.fav)}" ${current===g.fav?"selected":""}>${htmlEscape(g.fav)}</option>
      </select>
    </div>`;
  }).join("");
  const tbA=entry.tiebreak?.teamA??entry.tiebreak?.clemson??"";
  const tbB=entry.tiebreak?.teamB??entry.tiebreak?.lsu??"";
  panel.innerHTML=`
    <div class="edit-pick-head">
      <h4>Editing ${htmlEscape(name)}'s picks — ${htmlEscape(weekData?.label||currentWeekId)}</h4>
      <button type="button" id="cancelEditEntryPicks" class="link-btn">Cancel</button>
    </div>
    <div class="edit-pick-list">${gameRows}</div>
    ${tb?`<div class="edit-pick-tiebreak">
      <label class="score-input-label"><span>${htmlEscape(shortTeam(tb.dog))}</span><input type="number" min="0" id="editTbA" value="${htmlEscape(String(tbA))}"></label>
      <label class="score-input-label"><span>${htmlEscape(shortTeam(tb.fav))}</span><input type="number" min="0" id="editTbB" value="${htmlEscape(String(tbB))}"></label>
    </div>`:""}
    <div class="edit-pick-actions">
      <button type="button" id="saveEditEntryPicks" class="primary">Save Picks</button>
      <span id="editEntryPicksMsg" class="msg"></span>
    </div>
  `;
  $("cancelEditEntryPicks").onclick=()=>{panel.hidden=true;panel.innerHTML="";};
  $("saveEditEntryPicks").onclick=async()=>{
    const btn=$("saveEditEntryPicks");
    btn.disabled=true;btn.textContent="Saving…";
    try{
      const newPicks={};
      panel.querySelectorAll("[data-pick-game]").forEach(sel=>{
        if(sel.value) newPicks[sel.dataset.pickGame]=sel.value;
      });
      const update={picks:newPicks,updatedAt:serverTimestamp(),editedByAdmin:true};
      if(tb){
        const aVal=$("editTbA")?.value, bVal=$("editTbB")?.value;
        if(aVal!==""&&bVal!==""&&aVal!=null&&bVal!=null){
          update.tiebreak={teamA:Number(aVal),teamB:Number(bVal),teamAName:tb.dog,teamBName:tb.fav,gameId:tb.id};
        }
      }
      await setDoc(ref,update,{merge:true});
      seasonDataCache=null;
      $("editEntryPicksMsg").textContent="Saved.";
      await loadTracking({skipScoreRefresh:true});
      await loadCommissionerDashboard();
      if(currentWeekId===weekData?.id) { await loadPicks(); await updateWeeklyPersonalSummary(); }
      setTimeout(()=>{panel.hidden=true;panel.innerHTML="";},900);
    }catch(e){
      $("editEntryPicksMsg").textContent=`Could not save: ${e.message}`;
      btn.disabled=false;btn.textContent="Save Picks";
    }
  };
}

async function clearAllEntriesForWeek(){
  if(profile?.role!=="admin") return;
  const weekLabel=weekData?.label||currentWeekId;
  const snap=await getDocs(collection(db,"weeks",currentWeekId,"entries"));
  const count=snap.docs.length;
  if(!count){
    if($("adminEntryDeleteMsg")) $("adminEntryDeleteMsg").textContent="There are no entries for this week to clear.";
    return;
  }
  const ok=window.confirm(`Delete ALL ${count} submitted ${weekLabel} entries?\n\nThis is meant for re-running a corrected historical import — it permanently removes every player's picks and tiebreaker for this week so the next import starts from a clean slate. This cannot be undone.`);
  if(!ok) return;
  ...
  try{
    ...
    if($("adminEntryDeleteMsg")) $("adminEntryDeleteMsg").textContent=`Cleared ${count} entries for ${weekLabel}. You can now re-run the historical import or have players resubmit.`;
    await loadCommissionerDashboard();
    if(currentWeekId===weekData?.id) await loadTracking({skipScoreRefresh:true});
  }catch(e){
    if($("adminEntryDeleteMsg")) $("adminEntryDeleteMsg").textContent=`Could not clear entries: ${e.message}`;
  }finally{
    if(btn){btn.disabled=false;btn.textContent="Clear All Entries This Week";}
  }
}
if($("clearAllEntries")) $("clearAllEntries").onclick=clearAllEntriesForWeek;
    ...
    if($("adminSubmittedEntries")){
      if(!submittedEntries.length){
        ...
        $("adminSubmittedEntries").innerHTML=submittedEntries.map(e=>{
          ...
          return `<div class="submitted-entry-row"><div class="submitted-entry-person"><strong>${htmlEscape(name)}</strong><span>${htmlEscape(email)}</span><small>${htmlEscape(source)} · ${htmlEscape(when)}</small></div><div class="submitted-entry-actions"><button class="edit-entry-btn" type="button" data-edit-entry="${htmlEscape(e._docId)}">Edit Picks</button><button class="danger-entry-btn" type="button" data-delete-entry="${htmlEscape(e._docId)}">Delete Entry</button></div></div>`;
        }).join("");
        ...
        $("adminSubmittedEntries").querySelectorAll("[data-edit-entry]").forEach(btn=>{
          btn.onclick=()=>openEditEntryPicks(btn.dataset.editEntry);
        });
      }
    }
  }catch(e){
...
function renderAdminResults(){
  ...
  $("adminResultsGames").innerHTML=gs.map((g,i)=>{
    ...
    return `<div class="admin-result-row"><div class="admin-result-game"><strong>${i+1}. ${matchupDisplayNames(g).first} vs ${matchupDisplayNames(g).second}</strong><span>${g.fav} ${g.spread} · ${g.points||1} pt${Number(g.points||1)!==1?"s":""}</span></div><select data-result-game="${g.id}"><option value="" ${!outcome?"selected":""}>Pending</option><option value="${g.fav}" ${outcome===g.fav?"selected":""}>${g.fav} covers</option><option value="${g.dog}" ${outcome===g.dog?"selected":""}>${g.dog} covers</option><option value="PUSH" ${outcome==="PUSH"?"selected":""}>Push</option></select><input data-final-game="${g.id}" value="${String(finalText).replace(/&/g,"&amp;").replace(/\"/g,"&quot;")}" placeholder="Final score (optional)"></div>`;
  }).join("");
}
...
function renderResults(){
  ...
  $("resultsView").innerHTML=gs.map(g=>{
    ...
    return `<div class="result-row ${feedState}"><span><strong>${matchupDisplayNames(g).first} vs ${matchupDisplayNames(g).second}</strong><br><small>${g.fav} ${g.spread}</small></span><span class="result-status-block"><b class="result-status ${feedState}">${state}</b><span class="result-final">${score}${outcome?` · ${outcome==="PUSH"?"Push":`${outcome} covers`}`:""}</span></span></div>`;
  }).join("");
}
